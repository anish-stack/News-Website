...
const Headlines = () => {
  ...
  const fetchHeadlines = async (page = 1) => {
    try {
      const response = await axios.get('https://news-website-xaut.onrender.com/api/news/admin-headlines', {
  ...
  const createHeadline = async () => {
    try {
      const response = await axios.post('https://news-website-xaut.onrender.com/api/news/headlines', { headlineText: newHeadline });
  ...
  const updateHeadline = async () => {
    try {
      const response = await axios.put(`https://news-website-xaut.onrender.com/api/news/headlines/${editId}`, { headlineText: editHeadlineText });
      fetchHeadlines(currentPage); // Refresh the headlines
      setEditId(null);
      setEditHeadlineText('');
    } catch (error) {
  ...
  const toggleActiveStatus = async (id, currentStatus) => {
    try {
      const response = await axios.put(`https://news-website-xaut.onrender.com/api/news/headlines/${id}`, { active: !currentStatus });
      fetchHeadlines(currentPage); // Refresh the headlines
    } catch (error) {
  ...
  const deleteHeadline = async (id) => {
    try {
      await axios.delete(`https://news-website-xaut.onrender.com/api/news/headlines/${id}`);
      fetchHeadlines(currentPage); // Refresh the headlines
    } catch (error) {
